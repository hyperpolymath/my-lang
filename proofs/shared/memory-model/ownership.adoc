...
If Ω ⊢ e : τ and e ⟶ e’, then there exists Ω’ such that Ω’ ⊢ e’ : τ and
Ω’ respects the ownership invariants.

*Proof*: By induction on the typing derivation, showing that each
reduction step either: 1. Preserves ownership (no moves occur) 2.
Correctly transfers ownership (move semantics) 3. Creates valid borrows
(borrowing rules) □
...
*Proof*: - The typing rules require x ∉ moved(Ω) for variable access -
Move operations mark variables as moved - Subsequent access attempts
...
*Proof*: - Each value has exactly one owner (Invariant 1) - Only the
owner can free memory (via dropping) - After dropping, the owner is
removed from Ω - No other references to the freed memory exist
(Invariant 2) □
...
The ownership checker in `+src/checker.rs+` implements these rules
through:

[arabic]
. Tracking variable states (owned, borrowed, moved)
. Validating borrow exclusivity
