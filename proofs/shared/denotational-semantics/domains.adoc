...
A *complete partial order* (CPO) is a partially ordered set (D, ⊑) such
that:

[arabic]
. D has a least element ⊥ (bottom)
. Every ω-chain d₀ ⊑ d₁ ⊑ d₂ ⊑ … has a least upper bound ⊔ᵢdᵢ
...
*Examples*: - Z⊥ = \{⊥, …, -2, -1, 0, 1, 2, …} - B⊥ = \{⊥, true, false}
- String⊥ = \{⊥} ∪ String
...
[arabic]
. *Monotonic*: d ⊑ e implies f(d) ⊑ f(e)
. *Preserves lubs*: For every ω-chain \{dᵢ}, f(⊔ᵢdᵢ) = ⊔ᵢf(dᵢ)
...
If f : D → E and g : E → F are continuous, then g ∘ f : D → F is
continuous.
...
*Proof*: 1. The sequence \{fⁿ(⊥)} is a chain: fⁿ(⊥) ⊑ fⁿ⁺¹(⊥) by
monotonicity 2. Let d = ⊔ₙ fⁿ(⊥) 3. f(d) = f(⊔ₙ fⁿ(⊥)) = ⊔ₙ f(fⁿ(⊥)) =
⊔ₙ fⁿ⁺¹(⊥) = d 4. For any fixed point e, ⊥ ⊑ e, so fⁿ(⊥) ⊑ fⁿ(e) = e for
all n, hence d ⊑ e □
...
[arabic]
. *Bilimit construction*: Find D as the bilimit of an approximating
sequence
. *Category theory*: D is an initial algebra of functor F
...
Where: - Config = AI model configuration - Cache = Response cache - IO =
External computation monad
...
- Terminal object: 1 = \{⊥, ()} - Products: D × E - Exponentials: [D →
E]
...
If D and E are domains and f : D → E, g : E → D are continuous with g ∘
f = id_D and f ∘ g = id_E, then D ≅ E.
