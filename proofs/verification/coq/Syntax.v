...
(* Capture-avoiding substitution.
   subst x v e — replace free occurrences of x with v in e.
   Note: EMatch arms and EBlock statements are left unchanged because
   their binder structure requires pattern/statement-level substitution
   which is out of scope for the core expression substitution lemma.
   This is sound because has_type has no typing rules for EMatch, EBlock,
   EField, or ERecord, so those cases never arise in typing derivations. *)
Fixpoint subst (x : string) (v : expr) (e : expr) : expr :=
  ...
  | EArray es =>
      EArray ((fix subst_list (l : list expr) : list expr :=
        match l with
        | [] => []
        | e' :: rest => subst x v e' :: subst_list rest
        end) es)
  | _ => e  (* ERecord, EField, EMatch, EBlock, EAI: no typing rules, safe to leave unchanged *)
  end.
