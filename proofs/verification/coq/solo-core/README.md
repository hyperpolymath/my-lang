...
| `SessionPi.v` | **Ensemble session-π core + duet by projection + n-party projectability + static n-party config + label-union merge + union-projection + n-ary located opsem + head-coupled message subject reduction (axis-2 STRUCTURE, S1.0–S1.3 + S2.0–S2.2 + S3a + S3b + S3c.0 + S3c.1 + S3c.2 + S3c.3-msg).** A *standalone* development (NOT an extension of `SoloCore.v` — processes are a different term language, nothing carrier-bearing imported). **S1:** base-value payloads, session types `sty` with computed duality + `dual_involutive`, polarised endpoints, processes, a linear channel-typing judgement `wt` with context splitting, small-step `step`; value substitution (`wt_subst`) + communication-redex subject reduction (`sr_comm`); full closed-system subject reduction (`config_subject_reduction`) via the fused two-party `(νc)(P∣Q)` form; session fidelity (`session_fidelity`) + progress/deadlock-freedom (`config_progress`); **S1.3** n-ary choice (`SSelect`/`SBranch`), structural congruence (`wt_congr`), and the equi-recursive μ type-layer (`SMu`/`unfold_mu`/`dual_unfold`/`guarded`). **S2:** multiparty-*shaped* global types `gty` (message + **choice `GBra`** + **recursion `GMu`/`GVar`**), the now-**partial** projection `proj : gty → role → option sty` (three-way mutual; sender→`SSelect`, receiver→`SBranch`, uninvolved→**`merge`**, keystone `merge_idem`), two-party restriction `two_party`, `projection_duality` in **option-map form** (projection yields DUAL local types, mutual `two_party_mut` scheme), and `projected_config_wf` + corollaries transporting the whole S1 guarantee across projection where both projections are `Some` (deadlock-free by construction). **S3a:** the first n≥3 theorem — `projection_total : projectable_wf G → ∀ r:role, ∃ s, proj G r = Some s` (every role of a well-branched global type projects), via the mutual `projectable_wf_mut` scheme; non-vacuity at n=3 (3-party ring + agreeing 3-party choice merged), plain-merge boundary witnessed. **S3b:** the first n-party CONFIGURATION form — `role_assignment := list (role*party)` + the In-based **`wf_assignment G ra`** (every listed endpoint typed at its role's projection; a plain `Definition`, NOT nth/length, NOT a Fixpoint/Inductive), with the binary `Conf`/`wf_config` recovered as the **n=2 slice** (`conf_is_role_assignment2`, reusing `projected_config_wf`) + the converse embed `role_assignment2_of_conf` + the lookup bridge `ra_get`/`ra_get_in`, an n=3 non-`Conf` static witness over `g_ring` (`wf_ra_ring`), and honesty witnesses (duplicate-key tolerance, `projectable_but_uncoverable`). **S3c.0:** the Honda-Yoshida-Carbone label-**UNION** merge `umerge`/`umerge_br` (keystone `umerge_idem`), **added ALONGSIDE** the plain `merge` — at an external `&` choice a merged role offers the UNION of the branches' labels (the message fragment + internal `⊕` require EQUALITY via `sty_eqb`); the WIN is witnessed (a different-label `&` plain `merge` rejects, `umerge` unions: `umerge_widens_strictly`). **S3c.1:** a SEPARATE union-projection `proj_u`/`proj_br_u`/`proj_uninv_u` (the plain `proj` with the uninvolved fold's `merge` replaced by `umerge`), **added ALONGSIDE** `proj` (which is **NOT** re-pointed — first 2377 lines byte-identical); the union projectability predicate `projectable_u_wf`; the keystone existence theorem `projection_total_u` (the union analogue of `projection_total`, same mutual scheme); and the one-directional monotonicity bridge `projectable_wf_implies_u` (every plain-projectable `G` is union-projectable, projections coinciding via `proj_agrees_u`) riding on the keystone `merge_forces_eq` (plain merge is the identity-meet). Non-vacuity `g_union3` (uninvolved role 2 sees `&{3:end}` vs `&{4:end}` — different **labels**) is genuinely in *union-projectable minus plain-projectable*. **S3c.2:** the first OPERATIONAL metatheory over an n-party config — a located reduction `nstep` (the located mirror of the fused binary `cstep`: one communication `ra_set`-updates exactly the two communicating roles; `NStep_Comm`/`NStep_Sel`) + the functional update `ra_set` + a global-type reduction `gstep` + closures `nstar`/`gstar` + the functional well-formedness `wf_assignment_f` (with `wf_assignment_to_f` keeping S3b green). The 3-party ring run `0→1→2→0` to all-`QEnd` is witnessed on both sides (`ring_runs_to_end`, `g_ring_gsteps`); `nstep_length`/`nstar_length` show communication preserves the role count. The load-bearing **`nstep_breaks_wf_at_fixed_G`** *proves* `nstep` does not preserve `wf_assignment_f` at a fixed `G` — adequacy only, NO subject reduction (that couples `gstep`/`nstep` against a stepping `G` = S3c.3), NO progress (= S3c.4). **S3c.3-msg:** the first EARNED n-party safety half — **`nstep_sr_msg_head`** proves the **head** communication carries wf from `GMsg p q t G'` to its continuation `G'` (3-way located role case-split; the SAME `v:t` the sender ships is what the receiver substitutes via `pty_subst0`); coupled corollary `nstep_gstep_sr_msg_head` packages `gstep`+`nstep`+`wf'`. The SAME `ra_ring1` S3c.2 refuted at the fixed `g_ring` IS wf at the *stepped* `g_ring` (`ra_ring1_wf_at_stepped_g`, through the core SR; sealed by `sr_earns_safety_across_step`). The run-ahead fence is **self-witnessed** (`runahead_breaks_head_coupling`: a non-head step breaks head-continuation wf). HEAD message fragment only — run-ahead/permutation (S3c.3-perm), select (S3c.3-choice), progress (S3c.4) are OUT; preservation only. Adversary-verified *sound-and-honest*. Fences: **plain** (not label-union) merge, **unpruned** μ projection, **no** global-level metatheory; `projectable_wf` = projection EXISTENCE not safety; `two_party → projectable_wf` not proved (false); `wf_assignment` = typed-at-projection (STATIC) not n-party safety, `role_assignment` an association list (duplicate-tolerant, no coverage claim); `umerge` is TYPE-ALGEBRA only (canonical `proj`/coherence/`cstep` UNCHANGED — S3c.1 wires `umerge` into a SEPARATE `proj_u`, not into `proj`), NO safety claim, `⊕` not unioned, does NOT unlock the different-PAYLOAD `g_excluded` class (unions labels not payloads), only idempotence proved; `proj_u`/`projectable_u_wf`/`projection_total_u` are projection-EXISTENCE only (NO n-party safety), the monotonicity bridge is DOMAIN-INCLUSION (one-directional, converse FALSE via `g_union3`); the S3c.2 located opsem (`nstep`/`gstep`) is ADEQUACY-only (no SR/progress — `nstep_breaks_wf_at_fixed_G` proves no fixed-`G` SR); n-party SR/progress (**S3c.3–.4**, coupled `gstep`/`nstep` vs a stepping `G`) / μ typing up-to-unfolding (S1.3b-meta) are OUT. | **machine-checked (Coq), axiom-free** |
