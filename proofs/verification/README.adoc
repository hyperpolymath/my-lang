...
____
*Authoritative status:* link:../STATUS.md[`+../STATUS.md+`] is the
single source of truth for what is actually proved. The roadmap to
AffineScript parity is
link:../ALIGNMENT-PLAN.md[`+../ALIGNMENT-PLAN.md+`]. The status table
further down this file is a convenience summary and defers to
`+STATUS.md+` on any disagreement.
____

=== Contents

[arabic]
. link:coq/[`+coq/+`] — General Coq core (`+Syntax.v+`, `+Typing.v+`:
non-quantitative, substitution lemma proved).
. link:coq/solo-core/[`+coq/solo-core/+`] — *Mechanised QTT affine
solo-core* (Coq track): semiring laws proved; progress/preservation
stated.
. link:idris/solo-core/[`+idris/solo-core/+`] — *Mechanised QTT affine
solo-core* (Idris2 track), mirroring AffineScript’s solo-core.
. `+properties/+` — Property-based test specifications.
...
[arabic]
. Choose a theorem from the paper proofs
. Implement in Coq or Lean
. Ensure compilation with CI
. Submit PR with proof walkthrough
