...
The *Solo* dialect is My Language’s text-based programming mode for
learners ages 13+, featuring explicit affine types, contracts, and
parsing operators.

=== Contents

[arabic]
. link:affine-types/[Affine Types] - Linear and affine type system
. link:contracts/[Contracts] - Design-by-contract verification
. link:parsing-theory/[Parsing Theory] - Cut operator and deterministic
parsing
...
* *Theorem 1 (Affine Type Safety)*: Resources are not used after
consumption
