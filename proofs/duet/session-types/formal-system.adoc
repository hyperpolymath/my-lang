...
*Proof*: By induction on typing derivation. Each communication rule
corresponds to the expected session type transition. □

==== Theorem 2 (Deadlock Freedom)

Well-typed sessions in the core calculus are deadlock-free.

*Proof*: Duality ensures that for every send there’s a matching receive,
and for every select there’s a matching branch. The linear use of
channels prevents resource conflicts. □

==== Theorem 3 (Progress)

If Δ ⊢ P and P is not 0, then either P can reduce or P is waiting on an
external input.
