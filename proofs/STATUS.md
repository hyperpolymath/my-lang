...
| Session-π subject reduction (`Δ ⊢ P`, `P → P'` ⟹ `Δ' ⊢ P'`) | Coq `solo-core/SessionPi.v` | **statement-only** (obligation) | **S1.1 (planned).** The headline metatheorem — well-typed processes step to well-typed processes (session types advance to their continuations on communication). Not yet stated mechanically; design scoped to the communication redex first. |
