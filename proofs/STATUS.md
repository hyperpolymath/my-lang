...
| Session-π **full closed-system** subject reduction (`Δ ⊢ P`, `P → P'` ⟹ `Δ' ⊢ P'`) | Coq `solo-core/SessionPi.v` | **statement-only** (obligation) | **S1.1b (planned).** Lift `sr_comm` to the whole `step` relation for closed systems (`wt [] [] P → step P P' → wt [] [] P'`), deriving duality from ν-binding via a context-consistency invariant. Needs the ν binder un-annotated + existential `WT_Res` (so the ν channel may re-type after a communication). |
