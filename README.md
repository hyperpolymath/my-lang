...
| **me → solo elaboration** (`elab`, `elab_data_check`) | *machine-checked (Coq)* — **M1.0 / M1.1 done**: the visual `me` surface ([`proofs/me/`](proofs/me/)) elaborates into the solo core. `elab : me_tm → tm` is a core-landing analogue of the paper `translate`, with `check`-executed `Example` witnesses for the linear-use constructs (M1.0) and the universal, axiom-free Visual-Soundness theorem `elab_data_check` for the **no-linear-use data fragment only** (M1.1; universal linear-use = M1.1b, open). For that fragment this is the first mechanised surface→core elaboration-correctness result in either sibling — AffineScript, as surveyed (`proofs/ALIGNMENT-PLAN.md` §1, AS@main 2026-06-02), is solo-only with no `me`-like dialect. |
