...
| **me → solo elaboration** (`elab`, `elab_data_check`) | *machine-checked (Coq)* — **M1.0 / M1.1 done**: the visual `me` surface ([`proofs/me/`](proofs/me/)) elaborates into the solo core. `elab : me_tm → tm` is the mechanised `translate`, with a battery of `check`-executed `Example` witnesses (M1.0) and the universal, axiom-free Visual-Soundness theorem `elab_data_check` for the no-linear-use fragment (M1.1). This is the **first mechanised surface→core elaboration-correctness result in either sibling** — AffineScript is solo-only and has no `me`-like dialect. |
