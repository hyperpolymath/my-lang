...
This is the Solo dialect core syntax, derived from `My grammar (solo).enbf`.
...
5. `+`, `-`
6. `*`, `/`
7. Unary: `-`, `!` (if supported in lexer)
8. Call `f(x)`, field access `x.y`

## Lexical Notes

- Whitespace is ignored except to separate tokens.
- Comments: `//` and `/* */`.
- Keywords: `fn`, `struct`, `effect`, `where`, `pre`, `post`, `invariant`,
  `comptime`, `let`, `mut`, `if`, `else`, `go`, `return`, `await`, `try`,
  `restrict`, `true`, `false`.
- Operators: `= + - * / == != < > && ||`.
