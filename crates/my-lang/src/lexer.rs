...
impl<'a> Lexer<'a> {
    ...
    fn keyword_or_ident(&self, s: &str) -> TokenKind {
        match s {
            ...
            "orchestrate" => TokenKind::Orchestrate,
...
mod tests {
    ...
    fn test_ensemble_keywords() {
        let source = "agent ensemble workflow consensus belief capabilities constraints stateless stateful singleton";
        let mut lexer = Lexer::new(source);
        let tokens = lexer.tokenize();

        assert_eq!(tokens[0].kind, TokenKind::Agent);
        assert_eq!(tokens[1].kind, TokenKind::Ensemble);
        assert_eq!(tokens[2].kind, TokenKind::Workflow);
        assert_eq!(tokens[3].kind, TokenKind::Consensus);
        assert_eq!(tokens[4].kind, TokenKind::Belief);
        assert_eq!(tokens[5].kind, TokenKind::Capabilities);
        assert_eq!(tokens[6].kind, TokenKind::Constraints);
        assert_eq!(tokens[7].kind, TokenKind::Stateless);
        assert_eq!(tokens[8].kind, TokenKind::Stateful);
        assert_eq!(tokens[9].kind, TokenKind::Singleton);
    }
