...
fn item_summary(item: &my_lang::TopLevel) -> String {
    match item {
        ...
        my_lang::TopLevel::Agent(a) => format!("agent {}", a.name.name),
        my_lang::TopLevel::Ensemble(e) => format!("ensemble {}", e.name.name),
        my_lang::TopLevel::Orchestrate(o) => format!("orchestrate {}", o.name.name),
    }
}
