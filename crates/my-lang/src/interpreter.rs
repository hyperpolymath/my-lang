    ...
    pub fn exec(&mut self, stmt: &Stmt) -> Result<Value, RuntimeError> {
        match stmt {
            ...
            Stmt::Belief { name, confidence, .. } => {
                // In interpreter, belief statements create a placeholder value with confidence
                let belief_val = Value::Float(*confidence);
                self.env.borrow_mut().define(name.name.clone(), belief_val);
                Ok(Value::Unit)
            }
        }
    }
